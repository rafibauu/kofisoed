...
describe('saga - auth.js', () => {
    ...
    it('process normal login', () => {
        ...
        expect(gen.next(expectedResponse).value).toEqual(expectedDispatchLoginSuccess);
        expect(gen.next().done).toEqual(true);
    ...
    it('process token login', () => {
        ...
        const expectedCallLoginForm = call(api.get, `login-link?token=${payload.token}`);
        expect(gen.next().value).toEqual(expectedCallLoginForm);
        const data = {
            "token-laravel": "TokenLaravelJWT",
            "token-firebase": "TokenFirebase",
    ...
    it('log out success', () => {
        ...
        const expectedLogoutSuccess = takeEvery(types.LOGOUT_SUCCESS, processLogoutSuccess)
